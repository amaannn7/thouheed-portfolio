...
function TimelineRow({
  company, location, period, role, description, tags, featured = false, delay = 0, index = 1,
}: {
  company: string; location: string; period: string; role: string;
  description: string; tags: string[]; featured?: boolean; delay?: number; index?: number;
}) {
  const ref = useScrollReveal();
  return (
    <div
      ...
      className="reveal group border-b border-gray-100 last:border-b-0 py-7 md:py-9"
    >
      <div className="flex items-start gap-4 md:gap-6">
        {/* Index */}
        <span className="hidden md:block text-[11px] font-bold tabular-nums select-none text-gray-200 w-6 shrink-0 pt-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1 mb-1.5">
            <h4 className="text-[18px] md:text-[20px] font-semibold tracking-tight text-[#1a1a1a] leading-snug">
              {company}
            </h4>
            {featured && (
              <span className="text-[9px] font-bold uppercase tracking-widest bg-[#1a1a1a] text-white px-2.5 py-[3px] rounded-full leading-none">
                Now
              </span>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 mt-4">
            {tags.map(t => (
              <span key={t} className="text-[10px] font-semibold uppercase tracking-wide px-2.5 py-0.5 rounded-full bg-gray-100 text-gray-400">
                {t}
              </span>
            ))}
          </div>
        ...
        <div className="text-right shrink-0 pt-1">
          <p className="text-[12px] font-semibold text-[#1a1a1a] whitespace-nowrap">{period}</p>
          <p className="text-[11px] text-gray-300 mt-0.5">{location}</p>
        </div>
      </div>
    </div>
  );
...
function ProjectCard({ label, title, desc, tags, image }: {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl h-full w-full cursor-pointer bg-black">
      <Image src={image} alt={title} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition-all duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent" />
      <div className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0 shadow-md">
        <ArrowUpRight className="w-3.5 h-3.5" />
      </div>
      <div className="absolute bottom-0 left-0 right-0 p-5">
        <span className="text-[9px] font-semibold uppercase tracking-widest text-white/40">{label}</span>
        <h5 className="text-white font-semibold text-[15px] mt-1 leading-snug">{title}</h5>
        <div className="mt-2 flex flex-col gap-2 max-h-0 overflow-hidden group-hover:max-h-24 transition-all duration-400 ease-in-out">
...
function VideoProjectCard({ label, title, desc, tags, video }: {
  ...
  return (
    <div
      className="group relative overflow-hidden rounded-2xl h-full w-full cursor-pointer bg-black"
      onMouseEnter={() => vidRef.current?.play()}
      onMouseLeave={() => { if (vidRef.current) { vidRef.current.pause(); vidRef.current.currentTime = 0; } }}
    >
      <video ref={vidRef} src={video} muted playsInline loop
        className="absolute inset-0 w-full h-full object-cover transition-all duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/15 to-transparent" />
      {/* Badge */}
      <div className="absolute top-4 left-4">
        <span className="inline-flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest bg-white/10 backdrop-blur-sm text-white/70 border border-white/15 px-2.5 py-1 rounded-full">
    ...
    <MobileMenuCtx.Provider value={{ open: mobileOpen, setOpen: setMobileOpen }}>
      <div className="min-h-screen bg-[#f8f9fa] text-[#1A1A1A] font-sans">
        ...
        <main className="w-full pt-20">
          ...
          {/* ── PORTFOLIO ── */}
          <section id="portfolio" className="py-24 px-4 md:px-8 bg-[#f8f9fa] border-t border-gray-100">
            <div className="max-w-7xl mx-auto">
              ...
              <Reveal>
                <div className="flex items-end justify-between mb-6 px-2">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-gray-400 inline-block"></span> Portfolio
                    </p>
                    <h2 className="text-4xl font-semibold leading-tight text-[#1a1a1a]">Selected Works</h2>
                  </div>
                  <a href="/work" className="hidden md:inline-flex items-center gap-1.5 text-[12px] font-semibold text-black border-b border-black pb-0.5 hover-underline transition-colors duration-200">
                    View All Work <ArrowUpRight className="w-3.5 h-3.5" />
                  </a>
                </div>
              </Reveal>

              {/* ── Row 1: Ford (wide) + Facewash (narrow, tall) ── */}
              <div className="flex flex-col md:flex-row gap-2.5 md:h-[460px]">
                <Reveal delay={0} className="h-[300px] md:h-full w-full md:flex-[3]">
                  <VideoProjectCard
                    label="Commercial Video — Tavisgo Motors"
                    title="Ford Bronco – Outer Banks Commercial"
                    desc="A high-impact cinematic commercial. A journey from urban confinement to untamed wilderness — freedom, power, and self-discovery."
                    tags={['After Effects', 'Motion Design', 'Cinematic']}
                    video="/Ford_Outer_Banks_Commercial.mp4"
                  />
                </Reveal>
                <Reveal delay={100} className="h-[260px] md:h-full w-full md:flex-[2]">
                  <ProjectCard
                    label="Product Design — British Cosmetics"
                    title="Prevense Facewash"
                    desc="Pure science meets everyday care. A premium label combining clinical credibility with minimal elegance."
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5 mt-2.5">
                <Reveal delay={0} className="h-[280px]">
                  <VideoProjectCard
                    label="Motion Ad — DR3"
                    title="DR3 Free Shipping Campaign"
                    desc="Kinetic typography, glitch cuts, and particle flows communicate zero cost, zero resistance."
                    tags={['Motion Design', 'Kinetic Typography', 'Ad Campaign']}
                    video="/freeshipping.mp4"
                  />
                </Reveal>
                <Reveal delay={100} className="h-[280px]">
                  <VideoProjectCard
                    label="Product & Motion — DR3"
                    title="DR3 Performance Sneaker"
                    desc="Velocity lines, glitch transitions, and fluid morphing shapes animate the sneaker as an extension of speed."
                    tags={['Product Design', 'Motion Graphics', '3D Render']}
                    video="/sneaker.mp4"
                  />
                </Reveal>
                <Reveal delay={200} className="h-[280px]">
                  <ProjectCard
                    label="Luxury Campaign — British Cosmetics"
                    title="Prevense Gold Range"
                    desc="Gold accents, premium finishes, and a coherent brand story that elevates into prestige skincare."
              ...
              {/* ── Row 3: UN Report (wide) + CTA panel ── */}
              <div className="flex flex-col md:flex-row gap-2.5 mt-2.5 md:h-[320px]">
                <Reveal delay={0} className="h-[280px] md:h-full w-full md:flex-[3]">
                  <ProjectCard
                    label="Publication Design — United Nations"
                    title="UN Report Cover & Template"
                    desc="A clean, globally aligned report identity — clarity, authority, and inclusivity at every layout."
                    tags={['Publication Design', 'Editorial', 'InDesign']}
                    image="/UN-1.jpg"
                  />
                </Reveal>
                <Reveal delay={150} className="h-[200px] md:h-full w-full md:flex-[2]">
                  <div className="h-full rounded-2xl border border-gray-200 p-7 flex flex-col justify-between bg-white hover:bg-gray-50 transition-colors duration-300">
                    <div>
                      <p className="text-[9px] font-semibold uppercase tracking-widest text-gray-400 mb-4">6 Projects &amp; Counting</p>
                      <p className="text-[#1a1a1a] text-xl font-semibold leading-snug">Brand, motion, packaging &amp; editorial — all in one place.</p>
                    </div>
                    <a href="/work" className="inline-flex items-center gap-2 text-[13px] font-semibold text-white bg-black rounded-full px-5 py-2.5 hover:bg-gray-800 transition-all duration-200 self-start">
                      View All Work <ArrowUpRight className="w-3.5 h-3.5" />
                    </a>
                  </div>
                </Reveal>
